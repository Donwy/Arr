...
public class NetworkUtils {
    ...
    private static final String ALIBABA_DNS = "223.5.5.5";

    public static int isMobileNet(){
        boolean netEnable = (isConnected() && isAvailableByPing());
        if (netEnable){
            boolean wifiEnable = getWifiEnabled() && isWifiConnected();
            if (!wifiEnable){
                int netType = getNetworkType();
                if ((netType & NetUtils.NETWORK_MOBILE) != 0){
                    boolean mobileEnable = getMobileDataEnabled();
                    boolean isMobile = isMobileData();
                    if (mobileEnable || isMobile){
                        return netType;
                    }
                }
            }
    ...
    public static boolean isAvailableByPing() {
        boolean able = isAvailableByPing(null);
        if (!able){
            able = isAvailableByPing(ALIBABA_DNS);
        }
        return able;
    }
    ...
    public static int getNetworkType() {
        int netType = NetUtils.NETWORK_NO;
        NetworkInfo info = getActiveNetworkInfo();
        if (info != null && info.isAvailable()) {
            if (info.getType() == ConnectivityManager.TYPE_ETHERNET) {
                netType = NetUtils.NETWORK_ETHERNET;
            } else if (info.getType() == ConnectivityManager.TYPE_WIFI) {
                netType = NetUtils.NETWORK_WIFI;
                ...
                switch (info.getSubtype()) {
                    ...
                    case TelephonyManager.NETWORK_TYPE_HSPAP:
                        netType = NetUtils.NETWORK_3G;
                        break;

                    case TelephonyManager.NETWORK_TYPE_IWLAN:
                    case TelephonyManager.NETWORK_TYPE_LTE:
                        netType = NetUtils.NETWORK_4G;
                        break;
                    default:
                        ...
                        if (!TextUtils.isEmpty(subtypeName)){
                            if (subtypeName.equalsIgnoreCase("TD-SCDMA")
                                ...
                                netType = NetUtils.NETWORK_MOBILE;
                            }
                        }
                        break;
                }
            } else {
                netType = NetUtils.NETWORK_UNKNOWN;
            }
        }
        return netType;
    }
